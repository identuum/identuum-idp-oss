...
// An importer outside identuum-idp-oss (an edition binary on net/http's
// ServeMux) serves the UI from its own fs.FS beside one route of its own:
// the boundary forwards to that route, and the shell answers the UI's
// client-side paths.
func Example() {
	api := http.NewServeMux()
	api.HandleFunc("GET /api/v1/edition", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprint(w, `{"edition":"ce"}`)
	})
	ui := fstest.MapFS{
	...
	// The edition's router: its own routes first, the UI for the rest.
