...
func TestEmbedFS_StableFirstAndLast(t *testing.T) {
	...
	assert.Equal(t, "0037_agent_communication_authorizations.sql", actual[len(actual)-1],
		"last OSS migration must be 0037_agent_communication_authorizations.sql")
}
...
func TestCurrent_MatchesCore(t *testing.T) {
	...
	assert.Equal(t, "0037", pkgmigrations.Current(),
		"pkg/migrations.Current must report the highest pinned version 0037")
}
