...
func TestEmbedFS_StableFirstAndLast(t *testing.T) {
	...
	assert.Equal(t, "0033_authcode_issued_tokens.sql", actual[len(actual)-1],
		"last OSS migration must be 0033_authcode_issued_tokens.sql")
}
...
func TestCurrent_MatchesCore(t *testing.T) {
	...
	assert.Equal(t, "0033", pkgmigrations.Current(),
		"pkg/migrations.Current must report the highest pinned version 0033")
}
