...
func TestEmbedFS_StableFirstAndLast(t *testing.T) {
	...
	assert.Equal(t, "0038_dpop_proof_replays.sql", actual[len(actual)-1],
		"last OSS migration must be 0038_dpop_proof_replays.sql")
}
...
func TestCurrent_MatchesCore(t *testing.T) {
	...
	assert.Equal(t, "0038", pkgmigrations.Current(),
		"pkg/migrations.Current must report the highest pinned version 0038")
}
