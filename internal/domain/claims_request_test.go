...
func TestParseClaimsRequest(t *testing.T) {
	cases := []struct {
		...
		{"unknown claim ignored, never an error", `{"userinfo":{"name":null,"picture":null,"given_name":{"essential":true}}}`, ClaimsRequest{UserInfo: []string{"name"}}, nil},
		...
		{"only unknown claims → empty request", `{"userinfo":{"picture":null}}`, ClaimsRequest{}, nil},
...
func TestClaimsRequest_TokensEncodeDecode(t *testing.T) {
	...
	if got := DecodeClaimsRequest(`{"userinfo":["picture","name"]}`); !reflect.DeepEqual(got, ClaimsRequest{UserInfo: []string{"name"}}) {
...
func TestIntersectConsentedClaims(t *testing.T) {
	...
	if got := IntersectConsentedClaims([]string{"picture"}, RoleOrgUser); got != nil {
