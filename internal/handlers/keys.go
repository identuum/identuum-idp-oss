...
// a new signing key via service.KeyService.Generate. RS256 is
// rejected at the handler boundary per the Identuum no-RS256-
// issuance policy.
//
...
func HandleGenerateSigningKey(deps KeysHandlerDeps) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Identuum no-RS256-issuance policy: RS256 verification-
		// capable keys MUST NOT be generated by this endpoint.
		switch req.Algorithm {
		case string(domain.KeyAlgorithmEdDSA), string(domain.KeyAlgorithmES256):
			// ok
		default:
			c.JSON(http.StatusBadRequest, gin.H{
				"error":              "unsupported issuance algorithm",
				"supported":          []string{string(domain.KeyAlgorithmEdDSA), string(domain.KeyAlgorithmES256)},
				"rs256_issuance_ban": true,
			})
			return
		}
