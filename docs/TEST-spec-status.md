...
|---|---|
| Per-role coverage of ALL functionality (site_admin / org_admin / org_user) | PARTIAL — endpoints are exercised, but the honest denominator is (functionality × role) CELLS, and the negative half (org_user especially) is thin. The role census (T3 below) fixes the denominator before more tests are written. |
