...
|---|---|
| Per-role coverage of ALL functionality (site_admin / org_admin / org_user) | PARTIAL, **81% — 239 of 294 cells under THE SMARTER COUNTER (owner ruling 2026-08-31)**: role cells only where user type matters (88 endpoints × 3 = 264; 226 covered — site_admin 66, org_admin 79, org_user 81), one "@ any" class cell for each public/M2M endpoint (13 of 30 covered), and the 15 browser-cookie session endpoints EXCLUDED BY NAME (covered by the dev-loop suite + UI floor). [ROW 201] pinned the org_user tenant-battery; [ROW 202] pins the model's other direction — both tenant principals refused across the whole golden-declared site-admin surface, auto-tracking the golden. Matrix machine-enforced every run (ROLE-MATRIX-1, ui FLOOR 62). Remaining: 38 role cells (mostly site_admin positives needing curated fixtures + authenticated-class stragglers) and 17 class cells. |
